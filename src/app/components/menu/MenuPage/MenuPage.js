    ...
    <>
      ...
      <main className="bg-[#fef6ee] min-h-screen text-brown-900 scroll-smooth">
        <div className="py-10 px-6 max-w-6xl mx-auto bg-[#fff8f0] rounded-xl shadow-lg">
          <h2 className="text-3xl font-bold mb-6 text-center text-[#723c16]">Our Menu</h2>
          ...
          <div className="mb-6 flex justify-center text-black">
            <input
              ...
              className="w-full max-w-md px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#a35e20]"
            />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
            {filteredMenu.length === 0 ? (
              <p className="text-center text-gray-600 col-span-full">No menu matches your search.</p>
            ) : (
              filteredMenu.map((item) => (
                ...
                >
                  ...
                  <p className="text-xs italic text-[#cba258]">{item.category?.name || item.category}</p>
                  <button
